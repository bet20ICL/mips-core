...
TESTBENCHES="test/${INSTRUCTION}*_harvard_tb.v"


for i in ${TESTBENCHES}
do
    set +e
    iverilog -Wall -g2012 -o test/$(basename ${i} .v)  ${i} ${INTERNAL_FILES_1} ${INTERNAL_FILES_2} 2> test/$(basename ${i} .v)_dump.log
    RESULT=$?
    set -e
    ...
    if [[ RESULT -ne 0 ]] ; then
        ...
        set +e
        ./test/$(basename ${i} .v) > test/$(basename ${i} .v).log
        OUTPUT=$?
        set -e
